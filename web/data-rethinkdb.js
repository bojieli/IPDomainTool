...
function wildcardQuery(matchField, matchValue, limitNum, cont){
    ...
    if (!matchValue.match(/[^*?]{4}.*\./)) {
        cont("Your query is too board. Please include at least 4 consecutive non-wildcard chars.");
        return;
    }
    ...
    if (query.listing == "no") {
        ...
        if (typeof matchValue == "object" && matchValue["$like"]) {
            wildcardQuery(matchField, matchValue["$like"], limitNum, function(err, array){
                ...
                var cond = matchCondMap(query.listing, {$ne: null});
                cond[matchField] = {$in: array};
                getBrief(query.listing, cond, query, cont);
            });
        }
        else {
            var cond = matchCondMap(query.listing, {$ne: null}); // target field should not be null
            cond[matchField] = matchValue;
            getBrief(query.listing, cond, query, cont);
        }
    }
}
